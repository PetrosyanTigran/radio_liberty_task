(function main() {
  ...
  const { applyStyles } = helpers();

  function addItems(_currentHash) {
    const list = document.getElementById('list');
    const tags = _currentHash.replace('#tags=', '');
...
function helpers() {
  ...
  function applyStyles(listOfTags) {
    const template = (name) => `li.${name}:hover {
              background-color: ${
                stringToColor(name) ? stringToColor(name) : '#0073CF'
              };
              color: white;
              cursor: pointer;
          }`;
